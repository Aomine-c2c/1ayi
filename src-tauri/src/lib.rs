...
fn update_config(state: State<AyisState>, api_base: String) -> Result<AyisConfig, String> {
    state.config.api_base = api_base;
...
fn toggle_devtools(app: AppHandle) -> Result<(), String> {
    if let Some(window) = app.get_webview_window("main") {
        let should_be_open = !window.is_devtools_active().map_err(|e| e.to_string())?;
        window
            .set_devtools(should_be_open)
